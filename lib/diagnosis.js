        ...
        function attempt() {
            ...
            .then(function(data) {
                try {
                    ...
                    // Strategy 4: Truncated-JSON recovery (max_tokens 截断导致 JSON 未闭合:
                    // 取最后一个完整闭合的 } 前缀解析, 保住已生成的字段如 dayOverview/highlights)
                    if (!parsed && content.indexOf('{') === 0) {
                        var lastIdx = content.lastIndexOf('}');
                        if (lastIdx > 0) {
                            try {
                                parsed = JSON.parse(content.substring(0, lastIdx + 1));
                                if (parsed) console.warn('[Diagnosis] Bailian JSON truncated, recovered prefix (尾部字段可能缺失)');
                            } catch (_) { parsed = null; }
                        }
                    }
