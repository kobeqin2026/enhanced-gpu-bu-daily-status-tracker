...
function callBailian(prompt, systemPrompt) {
    return new Promise(function(resolve, reject) {
        ...
        // 间歇性空返回(LLM 返回内容为空/解析失败)自动重试, 最多3次
        var MAX_ATTEMPTS = 3;
        var attempts = 0;
        ...
        function attempt() {
            ...
            console.log('[Diagnosis] Calling Bailian (attempt ' + attempts + '/' + MAX_ATTEMPTS + '):', MODEL, 'at', apiUrl);
            ...
            .then(function(data) {
                try {
                    ...
                    // Strategy 4: Truncated-JSON repair (max_tokens 截断 → 末尾补引号+补闭合括号栈恢复,
                    // 保住已生成的字段如 dayOverview/perDomains)
                    if (!parsed && content.indexOf('{') === 0) {
                        var _inStr = false, _stack = [];
                        for (var _i = 0; _i < content.length; _i++) {
                            var _ch = content[_i];
                            if (_inStr) {
                                if (_ch === '\\') _i++;
                                else if (_ch === '"') _inStr = false;
                            } else {
                                if (_ch === '"') _inStr = true;
                                else if (_ch === '{' || _ch === '[') _stack.push(_ch);
                                else if (_ch === '}' || _ch === ']') {
                                    var _exp = (_ch === '}') ? '{' : '[';
                                    if (_stack.length && _stack[_stack.length - 1] === _exp) _stack.pop();
                                }
                            }
                        }
                        var _tail = _inStr ? '"' : '';
                        for (var _j = _stack.length - 1; _j >= 0; _j--) _tail += (_stack[_j] === '{' ? '}' : ']');
                        if (_tail) {
                            try {
                                parsed = JSON.parse(content + _tail);
                                if (parsed) console.warn('[Diagnosis] Bailian JSON truncated, repaired by closing brackets (尾部字段可能缺失)');
                            } catch (_) { parsed = null; }
                        }
                    }
            ...
            .catch(function(err) {
                // 空返回/解析失败属于可重试的间歇性故障; 网络/超时错误也重试一次
                var retryable = err.message === 'LLM 返回内容为空' || /解析响应失败/.test(err.message) || /abort|timeout|fetch failed/i.test(err.message);
                if (retryable && attempts < MAX_ATTEMPTS) {
                    var delay = 1200 * attempts;
                    console.warn('[Diagnosis] Bailian attempt ' + attempts + ' failed (' + err.message + '), retrying in ' + delay + 'ms');
 ...
 * @param {string} bugKey - JIRA issue key, e.g. EXAMPLE-123
...
function summarizeDailyDay(dayMarkdown) {
    var prompt = '你是GPU芯片Bring-up团队的测试负责人。下面是BU执行期内**同一天**的多个时刻快照（每个快照是该时刻的Bring-up状态，包含AI总体总结与规则明细）。' +
        ...
        '5. perDomains: 对当天**有进度记录**的每个domain做全天归纳，数组元素为 {"domain": "域名", "summary": "该domain全天进展1-3句(含测试动作/结果/阻塞/状态变化, 不罗列每条记录)"}；没有进度的domain不要出现在数组中\n' +
        '6. 只使用输入中出现的信息，不要编造，使用中文\n\n' +
        '全天快照输入：\n' + dayMarkdown + '\n\n' +
        '请只输出JSON，不要输出其他内容: {"dayOverview": "全天进展归纳", "highlights": ["进展1", "进展2"], "risks": ["风险1"], "nextSteps": ["建议1"], "perDomains": [{"domain": "PCIE", "summary": "PCIE全天..."}]}';
    ...
    return callBailian(prompt, '你是一个严谨的GPU芯片Bring-up测试负责人，擅长归纳多时刻快照为全天进展汇总。只输出JSON，不要任何解释。')
        .then(function(parsed) {
            ...
            var perDomains = [];
            try {
                if (Array.isArray(parsed && parsed.perDomains)) {
                    parsed.perDomains.forEach(function(pd) {
                        if (pd && pd.domain && pd.summary) {
                            perDomains.push({ domain: String(pd.domain).trim(), summary: String(pd.summary).trim() });
                        }
                    });
                }
            } catch (e) { perDomains = []; }
