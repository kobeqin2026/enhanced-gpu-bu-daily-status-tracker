...
function getDefaultUsers() {
    ...
    var CHANGE_MEPassword = process.env.DEFAULT_ADMIN_PASSWORD;
    var userPassword = process.env.DEFAULT_USER_PASSWORD;
    
    // Generate random passwords if env vars not set (first run)
    if (!CHANGE_MEPassword) {
        CHANGE_MEPassword = 'Admin' + crypto.randomBytes(8).toString('hex') + '!';
        console.log('[SECURITY] DEFAULT_ADMIN_PASSWORD not set, generated random password: ' + CHANGE_MEPassword);
    ...
    return [
        { id: 'CHANGE_ME', username: 'CHANGE_ME', password: CHANGE_MEPassword, role: 'CHANGE_ME', name: '管理员', createdAt: new Date().toISOString() },
