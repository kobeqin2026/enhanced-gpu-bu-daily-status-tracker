...
var VLM_MODEL = process.env.VLM_MODEL || 'qwen3.6-plus';

// 分析结果缓存 (imageUrl -> {summary, timestamp})
var analysisCache = {};
...
function analyzeImage(imageUrl, authHeader) {
    ...
    if (cached && (Date.now() - cached.timestamp) < CACHE_TTL) {
        console.log('[Vision] Cache hit for', imageUrl.substring(imageUrl.lastIndexOf('/') + 1));
    ...
    return callVisionAPI(imageUrl, authHeader);
}
...
function callVisionAPI(imageUrl, authHeader) {
            ...
            resp.on('end', function() {
                try {
                    ...
                    try {
                        ...
                        if (parsed.key_data && parsed.key_data.length > 0) {
                            summary += ' | 关键数据: ' + parsed.key_data.join(', ');
                        }
                        if (parsed.technical_details) {
                            summary += ' | 详情: ' + parsed.technical_details.substring(0, 300);
                        }
                        if (parsed.keywords && parsed.keywords.length > 0) {
                            summary += ' | 关键词: ' + parsed.keywords.join(', ');
                        }
                    } catch(jsonErr) {
                    ...
                    // 缓存
                    analysisCache[originalUrl] = { summary: summary, timestamp: Date.now() };
                    resolve(summary);
                } catch (e) {
        ...
        var req = client.request({
            ...
            resp.on('end', function() {
                try {
                    ...
                    analysisCache[imageUrl] = { summary: summary, timestamp: Date.now() };
                    resolve(summary);
                } catch (e) {
