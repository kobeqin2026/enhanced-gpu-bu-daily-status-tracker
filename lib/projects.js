...
/**
 * Loads the daily-summary history for a project, sorted by date ascending (递增).
 * @param {string} projectId - The identifier of the project.
...
async function loadDailySummaries(projectId) {
    ...
    try {
        ...
        return arr.sort(function(a, b) { return String(a.date || '').localeCompare(String(b.date || '')); });
    } catch (e) {
...
/**
 * Upserts one daily-summary record keyed by project+date (同日期重新生成则覆盖更新).
 * @param {string} projectId - The identifier of the project.
 * @param {object} record - { date, aiFailed, skeleton, ai, generatedBy, projectId }
 * @returns {Promise<{record:object, mode:string}>} mode = 'created' | 'updated'
 */
async function upsertDailySummary(projectId, record) {
    ...
    var idx = arr.findIndex(function(r) { return r.date === record.date; });
    var mode;
    if (idx >= 0) {
    ...
    var stored = arr.find(function(r) { return r.date === record.date; }) || arr[arr.length - 1];
    return { record: stored, mode: mode };
}
